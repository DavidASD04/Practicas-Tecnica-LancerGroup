    ...
    <div>
          ...
          <v-card>
            ...
            >              <template v-slot:item.nombre="{ item }">
              ...
              <template v-slot:item.apellido="{ item }">
                ...
                <Link :href="route('authors.show', item.id)" class="text-decoration-none">
                  <v-btn
                    icon="mdi-eye"
                    size="small"
                    color="info"
      ...
      <v-dialog v-model="deleteDialog" max-width="500px">
        <v-card>
          ...
          <v-card-text>
            ¿Estás seguro de eliminar al autor <strong>{{ selectedAuthor?.nombre }} {{ selectedAuthor?.apellido }}</strong>?
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
